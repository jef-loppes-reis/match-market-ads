...
class AnunciosLojaOficial:
    ...
    _link_num_pagina: tuple[str, dict[str, str]] = ('li', {'class': 'andes-pagination__button'})
    _anuncios: tuple[str, dict[str, str]] = ('ol', {'class': 'ui-search-layout ui-search-layout--grid'})
    _class_tag_mais_vendido: tuple[str, dict[str, str]] = ('div', {'class': 'ui-search-item__highlight-label__container'})
    _class_tag_avaliacao: tuple[str, dict[str, str]] = ('span', {'class', 'ui-search-reviews__rating-number'})
            ...
            for link_pagina in lista_paginas:
                ...
                for lista_anuncios in driver_anuncios.find_all(self._anuncios[0], self._anuncios[1]):
                    ...
                    for anuncio in lista_anuncios:
                        ...
                        #     'div', 'ui-search-item__highlight-label__container'
                        # )
                        _tag_mais_vendido: BeautifulSoup = anuncio.find(
                            self._class_tag_mais_vendido[0],
                            self._class_tag_mais_vendido[1]
                        )
                        _tag_mais_vendido_value: str | None = _tag_mais_vendido.text if not _tag_mais_vendido is None else None
                        ...
                        _tag_nota_avaliacao_value: float = float(_tag_nota_avaliacao.text) if not _tag_nota_avaliacao is None else 0.0
